...
class TestTrainModelPipeline:  # pragma: no cover
    def test_train(self, tmp_path):
        ...
        base_runs_folder = tmp_path / "runs"
        ...
        TrainModelPipeline(
            zip_path=zip_file_path, base_runs_folder=str(base_runs_folder)
        ).run()
        TrainModelPipeline(
            train_path=temp_train,
            test_path=temp_test,
            base_runs_folder=str(base_runs_folder),
        ).run()

        ckpt_path = base_runs_folder / "run_0" / "ingest" / "train_pipeline.ckpt"
        ckpt_pipe = TrainModelPipeline.load(ckpt_path)
        ckpt_pipe.resume()
        ckpt_pipe.tranform_predict(pd.read_csv(temp_test))
